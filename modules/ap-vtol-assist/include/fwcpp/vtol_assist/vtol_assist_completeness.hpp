...
inline constexpr VtolAssistPortItem kVtolAssistCompleteness[] = {
    ...
    {"altitude assist trigger", PortStatus::kRemaining,
     "Q_ASSIST_ALT + hysteresis (not stubbed)"},
    {"angle-error trigger", PortStatus::kRemaining,
     "Q_ASSIST_ANGLE envelope checks (not stubbed)"},
    {"should_assist full OR latch", PortStatus::kRemaining,
     "force||speed||alt||angle live flags + reset()"},
    {"check_VTOL_recovery", PortStatus::kRemaining, "fw recovery FSM"},
    {"output_spin_recovery", PortStatus::kRemaining, "rudder/elevator spin path"},
    {"logging/GCS getters", PortStatus::kRemaining,
     "in_*_assist + STATUSTEXT (not stubbed)"},
    {"Q_ASSIST_OPTIONS recovery paths", PortStatus::kRemaining,
     "FW_FORCE_DISABLED / SPIN_DISABLED"},
