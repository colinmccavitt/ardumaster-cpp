...
void EkfCore::covariance_prediction(const GyroSample& gyro, const AccelSample& accel, ftype dt_ekf_avg,
    ...
    if (quatCovResetOnly) {
        ...
        // CPP-077 phase 22: upstream calls calcTiltErrorVariance() at the
        // end of THIS early-return branch too (AP_NavEKF3_core.cpp ~line
        // 1436), not just at CovariancePrediction()'s normal end - both
        // real call sites are reproduced.
    ...
    // CPP-077 phase 22: upstream's real, unconditional end-of-
    // CovariancePrediction() call (AP_NavEKF3_core.cpp ~line 1791-1796:
    // `ConstrainVariances(); if (vertVelVarClipCounter > 0) {
    // vertVelVarClipCounter--; } calcTiltErrorVariance();`) - reproduced
    // in the same order. This is NOT new fusion-behavior wiring - it is
    // upstream's own real computation, which already runs every predict
    // cycle regardless of fusion/health state (see this file's own
    // "CPP-077, PHASE 22" banner in ekf_core.hpp).
    calc_tilt_error_variance();
}

// CPP-077, PHASE 22 (this ticket). upstream: NavEKF3_core::
// calcTiltErrorVariance(), AP_NavEKF3_core.cpp ~line 2154-2183. Its own
// upstream comment: "equations generated by quaternion_error_propagation():
// in derivation/generate_2.py" / "only diagonals have been used" - a real,
// dense, self-contained, auto-generated formula, verbatim-transcribed here
// exactly like the PS0..PS222 covariance-prediction block above (same
// naming convention/style, a GENUINELY SEPARATE set of local PS0..PS14
// intermediates - not the same variables, not reused). See ekf_core.hpp's
// own "CPP-077, PHASE 22" banner for the tilt_error_variance default-value
// reasoning.
void EkfCore::calc_tilt_error_variance() {
    // upstream: `const ftype &q0 = stateStruct.quat[0];` etc. This port's
    // QuaternionF stores the same w,x,y,z order as q1,q2,q3,q4 (see
    // ekf_core.hpp's StateVector banner: "quat 0..3" matches upstream's
    // own state-vector indexing exactly) - q1==upstream q0, q2==upstream
    // q1, q3==upstream q2, q4==upstream q3.
    const ftype& q0 = state.quat.q1;
    const ftype& q1 = state.quat.q2;
    const ftype& q2 = state.quat.q3;
    const ftype& q3 = state.quat.q4;

    // ---- BEGIN verbatim transcription of calcTiltErrorVariance()
    // (AP_NavEKF3_core.cpp ~line 2154-2183) ----
    ...
    tilt_error_variance = 4 * P[0][0] * sq(2 * PS8 - PS9) + 4 * P[1][1] * sq(2 * PS1 + PS3) +
                           4 * P[2][2] * sq(2 * PS4 + PS5) + 4 * P[3][3] * sq(-2 * PS6 + PS7);
    tilt_error_variance += 4 * P[0][0] * sq(2 * PS13 - PS7) + 4 * P[1][1] * sq(2 * PS14 - PS5) +
                           4 * P[2][2] * sq(2 * PS11 + PS3) + 4 * P[3][3] * sq(2 * PS12 + PS9);
    tilt_error_variance += 16 * P[0][0] * sq(PS14 - PS4) + 16 * P[1][1] * sq(PS13 + PS6) +
                           16 * P[2][2] * sq(-PS12 + PS8) + 16 * P[3][3] * sq(PS1 + PS11);
    // ---- END verbatim transcription ----

    tilt_error_variance = clamp(tilt_error_variance, ftype(0.0), sq(fwcpp::math::radians(ftype(30.0))));
}

// CPP-077, PHASE 22 (this ticket). upstream: NavEKF3_core::
// checkAttitudeAlignmentStatus(), AP_NavEKF3_Control.cpp ~line 509-527.
// EXCLUDED: the magYawResetRequest-triggering second half (`if
// (!yawAlignComplete && tiltAlignComplete && use_compass())
// magYawResetRequest = true;`) - ties to the yaw-reset machinery this port
// doesn't have (see ekf_core.hpp's own "CPP-077, PHASE 22" banner). NOT
// auto-invoked anywhere in this port yet - see that same banner for why.
void EkfCore::check_attitude_alignment_status() {
    if (!tilt_align_complete) {
        if (tilt_error_variance < sq(fwcpp::math::radians(ftype(5.0)))) {
            tilt_align_complete = true;
        }
    }
}
...
// ekf_core.hpp's own "CPP-077, PHASE 22" banner for the full branch
// investigation - this port dynamically selects upstream's real
// "!use_compass() and no GPS/EXTNAV yaw source" branch vs. its real
// "compass in use (or GPS-derived yaw)" branch using inhibit_mag_states as
// the honest, already-wired proxy for "is a yaw reference currently
// active", since this port has no yaw-source-selection machinery at all.
// NOT auto-invoked anywhere in this port yet - same banner explains why.
void EkfCore::check_gyro_cal_status(ftype dt_ekf_avg) {
    const ftype del_ang_bias_var_max = sq(fwcpp::math::radians(ftype(0.15) * dt_ekf_avg));
    if (inhibit_mag_states) {
        // upstream: rotate the bias-variance vector into earth frame and
        // evaluate horizontal terms only, since yaw is poorly observable
        // without a yaw reference.
        const Vector3F del_ang_bias_var_vec{P[10][10], P[11][11], P[12][12]};
        const Vector3F temp = prev_tnb * del_ang_bias_var_vec;
        del_ang_bias_learned = (std::abs(temp.x) < del_ang_bias_var_max) && (std::abs(temp.y) < del_ang_bias_var_max);
    } else {
        del_ang_bias_learned = (P[10][10] <= del_ang_bias_var_max) && (P[11][11] <= del_ang_bias_var_max) &&
                               (P[12][12] <= del_ang_bias_var_max);
    }
}
