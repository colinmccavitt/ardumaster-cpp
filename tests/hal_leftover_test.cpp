// CPP-088 slice 1: leftover catalog for remaining AP_HAL SITL surfaces.
...
TEST_CASE("hal leftover catalog lists this slice vs remaining", "[hal][leftover][catalog]") {
    ...
    REQUIRE(completeness_has("GPIO", PortStatus::kThisSlice));
    REQUIRE(completeness_has("Semaphore", PortStatus::kThisSlice));
    REQUIRE(completeness_has("BinarySemaphore", PortStatus::kThisSlice));
    REQUIRE(completeness_has("completeness catalog", PortStatus::kThisSlice));
    REQUIRE(completeness_has("I2CDevice", PortStatus::kRemaining));
    REQUIRE(completeness_has("SPIDevice", PortStatus::kRemaining));
    REQUIRE(completeness_has("Device register access", PortStatus::kRemaining));
