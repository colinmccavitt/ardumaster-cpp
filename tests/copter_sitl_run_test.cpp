// CCP-044 slice 2: copter_sitl_run leftover aero + arm/takeoff/hold/land catalog close.
...
using fwcpp::hal_sitl::copter_sitl_run::leftover_multirotor_aero;
...
TEST_CASE("leftover_multirotor_aero throttle zero stays on ground",
          "[copter][sitl][ccp-044]") {
    SimPlane sim{};
    REQUIRE(sim.on_ground());
    leftover_multirotor_aero(sim, 0.0f, 0.0025f);
    REQUIRE(sim.on_ground());
    REQUIRE((-sim.position.z) == Catch::Approx(0.0f).margin(0.01f));
}

TEST_CASE("leftover_multirotor_aero climb throttle leaves the ground",
          "[copter][sitl][ccp-044]") {
    SimPlane sim{};
    constexpr float kDt = 0.0025f;
    for (int i = 0; i < 1200; ++i) {  // 3 simulated seconds
        leftover_multirotor_aero(sim, 0.90f, kDt);
    }
    REQUIRE((-sim.position.z) > 2.0f);
    REQUIRE_FALSE(sim.on_ground());
}

TEST_CASE("leftover_multirotor_aero hover throttle holds altitude",
          "[copter][sitl][ccp-044]") {
    SimPlane sim{};
    sim.position.z = -10.0f;
    sim.velocity_ef = {};
    constexpr float kDt = 0.0025f;
    for (int i = 0; i < 400; ++i) {  // 1 simulated second
        leftover_multirotor_aero(sim, sim.hover_throttle, kDt);
    }
    REQUIRE((-sim.position.z) == Catch::Approx(10.0f).margin(0.5f));
}
...
TEST_CASE("leftover copter sitl mission arm takeoff hold land",
    ...
    bool saw_hold = false;
    const int max_ticks = 20 * 400;  // 20 simulated seconds
...
TEST_CASE("copter_sitl_run leftover catalog remaining_count",
    ...
    REQUIRE(completeness_has("leftover_multirotor_aero", PortStatus::kThisSlice));
    REQUIRE(completeness_has("leftover_mission_advance", PortStatus::kThisSlice));
    REQUIRE(completeness_has("leftover_hover_throttle", PortStatus::kThisSlice));
    ...
    REQUIRE(completeness_has("SIM_Multicopter Frame/Motor mixing", PortStatus::kOutOfScope));
}
