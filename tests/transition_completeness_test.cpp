...
TEST_CASE("quadplane transition catalog", "[transition][catalog]") {
    ...
    REQUIRE(completeness_has("SLT_Transition::update loop", PortStatus::kThisSlice));
    REQUIRE(completeness_has("Transition base-class virtuals", PortStatus::kThisSlice));
}
